...
df = pd.read_csv(r"C:\Users\joyju\Kizuna_Care\Data_Files\GA4_Non_Search_Session_Data.csv", skiprows = [x for x in range(0,8) if x!=6])
...
df.columns = ['date', 'session_source_medium', 'session_campaign', 'session_manual_ad_content', 'session_manual_term', 'sessions', 'engaged_sessions']
...
df.to_sql('ga4_non_search_session_data_temp', conn, if_exists = 'replace')

merge_query = '''
merge into ga4_non_search_session_data base
using ga4_non_search_session_data_temp new
...
when not matched then
    insert (date, session_source_medium, session_campaign, session_manual_ad_content, session_manual_term, sessions, engaged_sessions)

    values (new.date, new.session_source_medium, new.session_campaign, new.session_manual_ad_content, new.session_manual_term, new.sessions, new.engaged_sessions)
;
'''

cur.execute(merge_query)
cur.execute("drop table if exists ga4_non_search_session_data_temp")
